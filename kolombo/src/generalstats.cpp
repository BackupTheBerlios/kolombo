...
generalStats::generalStats(QWidget* parent, const char* name, bool modal, WFlags fl)
: parejasStatsDlg(parent,name, modal,fl)
{
	//http://doc.trolltech.com/3.3/tutorial2-05.html
	setCaption("Estadísticas generales");
	...
	m_canvas->setMaxY(200);
	m_canvas->setMargin(20, 35, 30, 25);
	m_canvas->setTitle ("Estadisticas generales (5 años)");
...
/*$SPECIALIZATION$*/
void generalStats::barras(int offset, int vuelo, int desaparecido, int reproduccion)
{
...
void generalStats::drawElements()
{
...
void generalStats::resizeEvent( QResizeEvent *e )
{
	if (firstResize)
	{
		m_canvas->resize(estadistica->width(), estadistica->height());
...
void generalStats::getData (int anyo, int *vuelo, int *desaparecido, int *reproduccion)
{
	QString consulta;
